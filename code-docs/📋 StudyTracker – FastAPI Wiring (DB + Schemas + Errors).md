# 📋 StudyTracker – FastAPI Wiring (DB + Schemas + Errors)

Refined FastAPI [`main.py`](http://main.py) that wires together **schemas**, **database + models**, and a basic **error envelope** pattern.

> This replaces the purely in-memory v0.1.0 version once you are ready to run against Postgres.
> 

---

## src/app/[main.py](http://main.py) (v0.2.0 – Postgres-backed)
...
@[app.post](http://app.post)("/topics", response_model=TopicRead, status_code=201)
async def create_topic(payload: TopicCreate, db: Session = Depends(get_db)):
    topic = TopicModel(name=[payload.name](http://payload.name), description=payload.description)
    db.add(topic)
...
async def get_topic(topic_id: int, db: Session = Depends(get_db)):
    topic = (
        db.query(TopicModel)
        .filter([TopicModel.id](http://TopicModel.id) == topic_id)
        .first()
    )
    if not topic:
...
@[app.post](http://app.post)("/sessions", response_model=SessionRead, status_code=201)
async def create_session(payload: SessionCreate, db: Session = Depends(get_db)):
    # Ensure topic exists
    topic = (
        db.query(TopicModel)
        .filter([TopicModel.id](http://TopicModel.id) == payload.topic_id)
        .first()
    )
    if not topic:
    ...
    session_obj = SessionModel(
        ...
        study_date=[payload.study](http://payload.study)_date,
...
You can keep the earlier in-memory version as `main_[inmemory.py](http://inmemory.py)` if you want a fallback for environments without Postgres.